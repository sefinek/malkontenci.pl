...
const generateCertificate = async ({ nickname, ticketNumber, score, arch }) => {
	...
	const [kicker, meta, dateLine, label, title, score_, diagnosis, treatLabel, treatment, sigPatient, sigDoctor, cornerNo, cornerCopy, finePrint, footer, seal, watermark] = await Promise.all([
		...
		renderTextCached({ text: 'Dokument został wygenerowany elektronicznie w systemie KPA i ważny bez podpisu. Diagnoza jest ostateczna i nie podlega reklamacji. Skargi na naszą skrzynke mailową nie będą nawet czytane.', size: '17px', color: COLORS.footer, width: CONTENT_W }),
...
const generateReferral = async ({ nickname, ticketNumber }) => {
	...
	const [kicker, meta, dateLine, label, title, facility, subtitle, details, body, note, sigDoctor, cornerNo, cornerCopy, finePrint, footer, seal, watermark] = await Promise.all([
		...
		renderTextCached({ text: 'Niniejszy dokument został wygenerowany elektronicznie w systemie KPA i ważny bez podpisu. Na izbę przyjęć prosimy zabrać dokument tożsamości oraz resztki dobrego humoru.', size: '17px', color: REFERRAL_COLORS.footer, width: CONTENT_W }),
